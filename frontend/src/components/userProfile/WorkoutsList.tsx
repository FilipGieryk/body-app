...
const WokroutsList = ({ userInfo }) => {
  return (
    <div className="flex flex-row w-[90%] h-[90%] flex-wrap content-start gap-4">
      <div className="bg-[url(./assets/brush-profile-horizontal.png)]  w-[100%] h-full bg-cover  absolute"></div>
      {userInfo.workouts.map((el) => (
        <Link className="h-20 rounded-4xl text-2xl" to={`/workouts/${el._id}`}>
          {el.name}
        </Link>
      ))}
      <Link
